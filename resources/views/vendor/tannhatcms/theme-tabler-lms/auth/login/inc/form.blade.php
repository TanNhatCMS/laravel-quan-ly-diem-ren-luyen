...
<form method="POST" action="{{ route('backpack.auth.login') }}" autocomplete="off" novalidate=""  data-pan="login-form">
    ...
    <div class="mb-3">
        ...
        <input autofocus tabindex="1" type="email" name="{{ $username }}" value="{{ old($username, 'admin@gmail.com') }}" id="{{ $username }}" class="form-control {{ $errors->has($username) ? 'is-invalid' : '' }}">
    ...
    <div class="mb-2">
        ...
        <input tabindex="2" type="password" name="password" id="password" class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" value="{{ old($username) ? '' : 'admin@1234' }}">
